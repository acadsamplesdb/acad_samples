...
# varietas????
    common = models.CharField("Common name", max_length=255)
...
class Sample(models.Model):
    ...
    other_num = models.CharField("Other sample number", max_length=255, blank=True)
    group = models.ForeignKey("AQISSampleGroup", blank=True, null=True, on_delete=models.SET_NULL)
    category = models.CharField("Category of sample", max_length=255)
    extracted_by_date = models.CharField("Extracted by/date", max_length=255, blank=True)
    acad_location = models.CharField("ACAD location", max_length=255, blank=True)
    date_of_entry = models.DateField("Date of entry into ACAD", blank=True, null=True)
    abc_num = models.CharField("ABC/Oxford sample number", max_length=255, blank=True)
    organism = models.ForeignKey(Organism, blank=True, null=True)
    common_name = models.CharField("Organism common name", max_length=255, blank=True)
    genus = models.CharField("Genus", max_length=255, blank=True)
    species = models.CharField("Species", max_length=255, blank=True)
    subspecies = models.CharField("Subspecies", max_length=255, blank=True)
    ...
    locality = models.CharField("Locality", max_length=255, blank=True)
    location = models.CharField("Specific location", max_length=255, blank=True)
    lat = models.CharField("Latitude", max_length=255, blank=True)
    lon = models.CharField("Longitude", max_length=255, blank=True)
    datum = models.CharField("Map or GPS datum from which lat/lon taken", max_length=255, blank=True)
    collection_date = models.DateField("Collection date", blank=True, null=True)
    collected_by = models.CharField("Collected by", max_length=255, blank=True)
    quality = models.CharField("Quality", max_length=255, blank=True)
    museum = models.CharField("Museum name", max_length=255, blank=True)
    museum_num = models.CharField("Museum accession number", max_length=255, blank=True)
    sampled_by = models.CharField("Sampled by", max_length=255, blank=True)
    sample_date = models.DateField("Date sampled", blank=True, null=True)
    mismatch_reason = models.CharField("Mismatch reason", max_length=255, blank=True)
    sample_repat = models.CharField("Sample repatriated", max_length=255, blank=True)
...
class Extraction(models.Model):
    ...
    extracted_by = models.CharField("Extracted by", max_length=255)
...
class ExtractResult(models.Model):
    ...
    starting = models.CharField("Starting material", max_length=255, blank=True)
    final_vol = models.CharField("Final DNA volume", help_text="(in ul)", max_length=255, blank=True)
    dna_yield = models.PositiveIntegerField(blank=True, null=True)
    quant_method = models.CharField("Quantification method", max_length=255, choices=QUANT_METHODS, blank=True)
...
class Amplification(models.Model):
    ...
    prepared_by = models.CharField("Prepared by", max_length=255)
    method = models.TextField("Method", help_text="e.g. library prep or PCR")
    amp_type = models.CharField("Amplification type", max_length=255, blank=True)
    treatment = models.CharField("Treatment", max_length=255, blank=True)
    notes = models.TextField(blank=True)
    file = models.ManyToManyField('FileAttachment', blank=True)
...
class AmplificationResult(models.Model):
    ...
    dna_yield = models.PositiveIntegerField(blank=True, null=True)
    quant_method = models.CharField("Quantification method", max_length=255, blank=True)
...
class EnrichmentResult(models.Model):
    ...
    enrich_type = models.CharField("Enrichment type", max_length=255, help_text="e.g. Mito, 10k", blank=True)
    bait_detail = models.CharField("Bait detail", max_length=255, help_text="MyBait batch number", blank=True)
    dna_yield = models.PositiveIntegerField(blank=True, null=True)
    quant_method = models.CharField("Quantification method", max_length=255, blank=True)
...
class Permit(models.Model):
    permit_num = models.CharField("Permit number", max_length=255)
    valid_from = models.DateField("Valid from")
    valid_to = models.DateField("Valid to")
    valid_for = models.CharField("Valid for", max_length=255)
    active_from = models.DateField("Active from", blank=True)
    conditions = models.TextField("Conditions", max_length=255, blank=True)
...
class AQISSampleGroup(models.Model):
    ...
    group_num = models.CharField("Group number", max_length=255, primary_key=True, blank=True)
    group_num_int= models.PositiveIntegerField(blank=True, null=True)
    aqis_num = models.CharField("AQIS Quarantine Entry/Order Number", max_length=255, blank=True)
    ...
    permit_num = models.CharField("Permit number", max_length=255, blank=True)
    description = models.CharField("Goods description", max_length=255, blank=True)
    quantity = models.PositiveSmallIntegerField("Quantity", blank=True, null=True)
    origin = models.CharField("Country of origin", max_length=255, blank=True)
    acad_loc = models.CharField("ACAD location", max_length=255, blank=True)
    movement = models.CharField("Movement and transfer", max_length=255, blank=True)
    notes = models.CharField("Other relevant info", max_length=255, blank=True)
    sent_by = models.CharField("Sent by", max_length=255, blank=True)
    contact = models.CharField("ACAD contact", max_length=255, blank=True)
...
class FileAttachment(models.Model):
    ...
    name = models.CharField("File name", max_length=255, blank=True)
...
class C14(models.Model):
    ...
    date = models.CharField("Estimated age in radiocarbon years", max_length=255)
    error = models.PositiveSmallIntegerField("Estimated carbon date error range", blank=True, null=True)
    centre = models.CharField("Centre that did carbon dating", max_length=255)
    centre_num = models.CharField("Centre reference number", max_length=255)
